...
if (JSON.stringify(manifest.permissions) !== JSON.stringify(['storage'])) {
  throw new Error('Extension permissions must remain limited to storage for local-first persistence.');
}

if (manifest.optional_permissions || manifest.host_permissions || manifest.optional_host_permissions || manifest.content_scripts) {
  throw new Error('Extension must not declare optional, host, or content-script permissions for V1.');
}

if (manifest.content_security_policy?.extension_pages !== "script-src 'self'; object-src 'self'") {
  throw new Error('Extension pages must retain the MV3-compatible self-only content security policy.');
}
...
const code = await Promise.all(scripts.map((file) => readFile(resolve('apps/extension/dist/assets', file), 'utf8')));
...
if (bundleBytes > 750_000) throw new Error(`Extension JavaScript bundle exceeds the 750 KB release budget (${bundleBytes} bytes).`);
console.log(`Extension policy and new-tab entry verified. JavaScript bundle: ${bundleBytes} bytes.`);
