...
export function codeforcesUrl(
  section: 'contests' | 'problemset' | 'profile' = 'contests',
  handle?: string,
): string {
  if (section === 'profile' && handle)
    return `${CODEFORCES_WEB_ROOT}/profile/${encodeURIComponent(handle)}`;
  if (section === 'problemset') return `${CODEFORCES_WEB_ROOT}/problemset`;
  return `${CODEFORCES_WEB_ROOT}/contests`;
}

export function getCodeforcesContestState(
  contest: CodeforcesContest,
  now = new Date(),
): CodeforcesContestState {
  const start = Date.parse(contest.startAt);
  if (!Number.isFinite(start) || now.getTime() < start || contest.phase === 'BEFORE')
    return 'upcoming';
  const finish = start + (contest.durationSeconds ?? 0) * 1_000;
  return now.getTime() < finish || contest.phase === 'CODING' ? 'running' : 'finished';
}

export function getNextCodeforcesContest(
  context: CodeforcesContext,
  now = new Date(),
): CodeforcesContest | undefined {
...
export async function fetchCodeforcesContext(
  ...
  if (handle && !normalizedHandle) {
    return {
      kind: 'error',
      error: providerError('invalid-handle', 'Enter a valid public Codeforces handle.', now()),
    };
  }
  ...
  const mappedContests = Array.isArray(contests.value)
    ? contests.value
        .flatMap(mapCodeforcesContest)
        .sort((left, right) => left.startAt.localeCompare(right.startAt))
    : [];
  if (!normalizedHandle) {
    return {
      kind: 'success',
      context: { contests: mappedContests, generatedAt: now().toISOString(), submissions: [] },
    };
  }

  await (options.wait ?? wait)(CODEFORCES_REQUEST_INTERVAL_MS);
  const user = await getApiResult(
    options,
    `user.info?handles=${encodeURIComponent(normalizedHandle)}`,
    now,
  );
  ...
  if (!profile)
    return {
      kind: 'error',
      error: providerError('invalid-handle', 'That Codeforces handle was not found.', now()),
    };

  await (options.wait ?? wait)(CODEFORCES_REQUEST_INTERVAL_MS);
  const submissions = await getApiResult(
    options,
    `user.status?handle=${encodeURIComponent(normalizedHandle)}&from=1&count=5`,
    now,
  );
  ...
  return {
    ...
    context: {
      ...
      profile,
      submissions: Array.isArray(submissions.value)
        ? submissions.value.flatMap(mapCodeforcesSubmission)
        : [],
    },
  };
}

export function readCodeforcesCachedContext(
  cache: IntegrationCacheState | undefined,
): CodeforcesContext | undefined {
  const value = cache?.entries.context?.value;
  return isCodeforcesContext(value) ? value : undefined;
}

export function saveCodeforcesCachedContext(
...
export function isCodeforcesCacheStale(cache: IntegrationCacheState, now = new Date()): boolean {
  ...
  return (
    ...
    now.getTime() - Date.parse(cachedAt) >= CODEFORCES_CACHE_STALE_AFTER_MS
  );
}
...
async function getApiResult(
  ...
  try {
    ...
    return {
      ...
      error: providerError(
        'network-error',
        'Codeforces could not be reached. Cached data remains available.',
        now(),
      ),
    };
  }
  if (!response.ok)
    return {
      ...
      error: providerError(
        'codeforces-response',
        'Codeforces could not load public data. Cached data remains available.',
        now(),
      ),
    };
  try {
    ...
    if (!isRecord(body) || body.status !== 'OK') {
      return {
        ...
        error: providerError(
          ...
          'Codeforces did not return public data. Cached data remains available.',
          now(),
        ),
      };
    }
    ...
    return {
      kind: 'error',
      error: providerError('invalid-response', 'Codeforces returned unreadable data.', now()),
    };
  }
}
...
function mapCodeforcesSubmission(value: unknown): CodeforcesSubmissionSummary[] {
  if (
    !isRecord(value) ||
    typeof value.id !== 'number' ||
    typeof value.creationTimeSeconds !== 'number'
  )
    return [];
  ...
  return [
    {
      ...
      id: value.id,
      problemName:
        typeof problem.name === 'string' ? problem.name.slice(0, 300) : 'Unknown problem',
