...
export function OrganizationManager({
  ...
  const title =
    screen === 'links' ? 'Quick links' : screen === 'projects' ? 'Projects' : 'Workspaces';

  return (
    <Dialog label={title} onClose={onClose} open={screen !== null}>
      {screen === 'links' && (
        <QuickLinksEditor onSettingsChange={onSettingsChange} settings={settings} />
      )}
      {screen === 'projects' && (
        <ProjectsEditor onSettingsChange={onSettingsChange} settings={settings} />
      )}
      {screen === 'workspaces' && (
        <WorkspacesEditor
...
function QuickLinksEditor({
  ...
  function save(event: FormEvent<HTMLFormElement>) {
    ...
    const next = editingId
      ...
      : (() => {
          ...
          return link &&
            !settings.quickLinks.some((existing) => existing.alias && existing.alias === link.alias)
  ...
  return (
    <section className="manager-section" aria-labelledby="quick-links-manager-title">
      ...
      <p className="muted">
        Keep your useful destinations nearby. Icons fall back to initials and never use remote
        favicon services.
      </p>
          ...
          <li key={link.id}>
            ...
            <Toggle
              ...
              onClick={() =>
                onSettingsChange({
                  ...settings,
                  quickLinks: settings.quickLinks.map((candidate) =>
            ...
            <span className="manager-actions">
              ...
              <Button onClick={() => edit(link)} variant="quiet">
                Edit
              </Button>
              <Button
                onClick={() =>
                  onSettingsChange({
                    ...settings,
                    quickLinks: removeQuickLink(settings.quickLinks, link.id),
                  })
                }
                variant="quiet"
              >
      ...
      <EditorForm title={editingId ? 'Edit quick link' : 'Add quick link'} onSubmit={save}>
        ...
        <Field label="Icon or initials (optional)">
        ...
        <FormActions
          ...
          onCancel={reset}
          submitLabel={editingId ? 'Save link' : 'Add link'}
        />
      </EditorForm>
    </section>
  );
...
function ProjectsEditor({
  ...
  function saveAction(event: FormEvent<HTMLFormElement>) {
    ...
    const action = createProjectAction(
      { kind: actionKind, label: actionLabel, url: actionUrl },
      actionId ?? makeId('project-action'),
    );
    const next = action && saveProjectAction(settings.projects, selected.id, action);
    if (!next) {
      setError('An action needs a label and a safe http or https URL.');
      return;
    }
    onSettingsChange({ ...settings, projects: next });
    setActionId(null);
  ...
  return (
    <section className="manager-section" aria-labelledby="projects-manager-title">
      ...
      <h2 id="projects-manager-title">Projects</h2>
            ...
            <span className="manager-actions">
              ...
              <Button
                onClick={() =>
                  onSettingsChange({
                    ...settings,
                    projects: removeProject(settings.projects, project.id),
                  })
                }
                variant="quiet"
              >
      ...
      {selected && (
        <EditorForm title="Project actions" onSubmit={saveAction}>
                ...
                <span>
                  ...
                  <Button
                    onClick={() =>
                      onSettingsChange({
                        ...settings,
                        projects: removeProjectAction(settings.projects, selected.id, action.id),
                      })
                    }
                    variant="quiet"
                  >
          ...
          <Field label="Action type">
            <select
              onChange={(event) => setActionKind(event.target.value as ProjectActionKind)}
              value={actionKind}
            >
              {(
                [
          ...
          <FormActions
            ...
            submitLabel={actionId ? 'Save action' : 'Add action'}
          />
        </EditorForm>
      )}
    </section>
...
function WorkspacesEditor({
  ...
  function save(event: FormEvent<HTMLFormElement>) {
    ...
    const next = editingId
      ? updateWorkspace(settings.workspaces, editingId, input)
      : (() => {
          const workspace = createWorkspace(input, makeId('workspace'), settings.workspaces.length);
          return workspace ? [...settings.workspaces, workspace] : null;
        })();
    if (!next) {
      setError('A workspace needs a name.');
      return;
    }
    onSettingsChange({ ...settings, workspaces: next });
    reset();
  }
  function edit(id: string) {
    const workspace = settings.workspaces.find((candidate) => candidate.id === id);
    if (!workspace) return;
    setEditingId(id);
    setName(workspace.name);
    setDescription(workspace.description ?? '');
    setShowOnHome(workspace.showOnHome);
    setError('');
  }
  function saveItem(event: FormEvent<HTMLFormElement>) {
    ...
    const item = createWorkspaceItem(
      { label: itemLabel, url: itemUrl },
      itemId ?? makeId('workspace-item'),
    );
    const next = item && saveWorkspaceItem(settings.workspaces, selected.id, item);
    if (!next) {
      setError('A destination needs a label and a safe http or https URL.');
      return;
    }
    onSettingsChange({ ...settings, workspaces: next });
    setItemId(null);
  ...
  return (
    <section className="manager-section" aria-labelledby="workspaces-manager-title">
      <p className="eyebrow">GROUPED LAUNCHES</p>
      <h2 id="workspaces-manager-title">Workspaces</h2>
      <p className="muted">Launching a workspace always confirms how many tabs will open.</p>
      <ul className="manager-list" aria-label="Workspaces">
        {[...settings.workspaces]
            ...
            <li key={workspace.id}>
              ...
              <span className="manager-actions">
                <Button
                  ...
                  onClick={() => onRequestWorkspaceLaunch(workspace)}
                >
                  Launch
                </Button>
                <Button onClick={() => edit(workspace.id)} variant="quiet">
                  Edit
                </Button>
                <Button
                  onClick={() =>
                    onSettingsChange({
                      ...settings,
                      workspaces: removeWorkspace(settings.workspaces, workspace.id),
                    })
                  }
                  variant="quiet"
                >
      ...
      <EditorForm title={editingId ? 'Edit workspace' : 'Add workspace'} onSubmit={save}>
        <Field label="Name">
          <TextInput onChange={(event) => setName(event.target.value)} required value={name} />
        </Field>
        <Field label="Description (optional)">
        ...
        <FormActions
          editing={Boolean(editingId)}
          onCancel={reset}
          submitLabel={editingId ? 'Save workspace' : 'Add workspace'}
        />
      </EditorForm>
      {selected && (
        <EditorForm title="Workspace destinations" onSubmit={saveItem}>
          <ul className="compact-list">
                ...
                <span>
                  ...
                  <Button
                    onClick={() =>
                      onSettingsChange({
                        ...settings,
                        workspaces: removeWorkspaceItem(settings.workspaces, selected.id, item.id),
                      })
                    }
                    variant="quiet"
                  >
          ...
          <FormActions
            ...
            submitLabel={itemId ? 'Save destination' : 'Add destination'}
          />
        </EditorForm>
      )}
    </section>
