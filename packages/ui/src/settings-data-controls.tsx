...
export function SettingsDataControls({
  ...
  function savePresets() {
    ...
    if (
      !values.length ||
      values.length > 5 ||
      values.some((value) => !Number.isInteger(value) || value < 1 || value > 180)
    ) {
    ...
    <>
      <section className="settings-section" aria-labelledby="home-sections-title">
              ...
              onClick={() =>
                update({
                  homeSections: {
                    ...
                    [section]: !settings.homeSections[section],
                  },
                })
              }
        ...
        <div className="form-actions">
          <Button onClick={() => onOpenOrganization('links')} variant="quiet">
            Manage quick links
          </Button>
          <Button onClick={() => onOpenOrganization('projects')} variant="quiet">
            Manage projects
          </Button>
          <Button onClick={() => onOpenOrganization('workspaces')} variant="quiet">
            Manage workspaces
          </Button>
          <Button onClick={() => update({ homeSections: DEFAULT_HOME_SECTIONS })} variant="quiet">
            Reset layout
          </Button>
        </div>
      </section>
      ...
      <section className="settings-section" aria-labelledby="focus-preferences-title">
        ...
        <label className="editor-field" htmlFor="focus-presets">
          Focus presets (comma separated minutes)
          <TextInput
            id="focus-presets"
        ...
        <div className="form-actions">
          ...
          <Button
            onClick={() => {
              setPresetText(DEFAULT_FOCUS_PRESETS.join(', '));
              update({ focusPresets: DEFAULT_FOCUS_PRESETS });
            }}
            variant="quiet"
          >
            Reset presets
        ...
        <fieldset className="provider-options">
          ...
          {(['system', 'reduce'] as const).map((preference) => (
            <Toggle
              key={preference}
              onClick={() => update({ reducedMotion: preference })}
              pressed={settings.reducedMotion === preference}
            >
      ...
      <section className="settings-section" aria-labelledby="activity-title">
        ...
        <p className="muted">
          Recent actions store only labels and timestamps, never search terms or snippet contents.
        </p>
        <div className="settings-toggles">
          <Toggle
            onClick={() => update({ recordRecentActions: !settings.recordRecentActions })}
            pressed={settings.recordRecentActions}
          >
        ...
        <div className="form-actions">
          ...
          <Button
            onClick={() => update({ recentExecutions: clearRecentExecutions() })}
            variant="quiet"
          >
      ...
      <section className="settings-section" aria-labelledby="backup-title">
        ...
        <p className="muted">
          Backups are JSON files you control. Import previews all replacement data before anything
          is changed.
        </p>
        <div className="form-actions">
        ...
        {importPreview && (
          <div className="import-preview" role="status">
            ...
            <span>
              {importPreview.quickLinks.length} links · {importPreview.projects.length} projects ·{' '}
              {importPreview.workspaces.length} workspaces · {importPreview.snippets.length}{' '}
              snippets
            </span>
            <div className="form-actions">
              <Button onClick={() => setImportPreview(null)} variant="quiet">
                Cancel import
              </Button>
              <Button
                onClick={() => {
                  onSettingsChange(importPreview);
                  setImportPreview(null);
                }}
              >
                Replace with backup
      ...
      <section className="settings-section" aria-labelledby="reset-title">
        ...
        <p className="muted">
          Section resets only affect local Navode data. Export a backup first if you may need it
          later.
        </p>
        <div className="form-actions">
          <Button onClick={() => update({ quickLinks: createStarterQuickLinks() })} variant="quiet">
            Reset quick links
          </Button>
          <Button
