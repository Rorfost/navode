...
describe('quick links', () => {
  it('creates, updates, deletes, reorders, and validates links', () => {
    const docs = createQuickLink({ alias: 'docs', name: 'Docs', url: 'https://example.com/docs' }, 'docs', 0)!;
    const tasks = createQuickLink({ name: 'Tasks', url: 'https://example.com/tasks' }, 'tasks', 1)!;

    expect(updateQuickLink([docs, tasks], 'docs', { alias: 'docs', name: 'Documentation', url: 'https://example.com/docs' })).toMatchObject([
      { id: 'docs', name: 'Documentation' },
    ]);
    expect(reorderQuickLinks([docs, tasks], ['tasks', 'docs']).map((link) => link.id)).toEqual(['tasks', 'docs']);
    ...
    expect(updateQuickLink([docs], 'docs', { alias: 'bad alias', name: 'Docs', url: docs.url })).toBeNull();
  });
});

describe('projects and workspaces', () => {
  it('stores editable project actions with validated URLs', () => {
    const project = createProject({ name: 'Documentation site', description: 'Generic example' }, 'project-1')!;
    const action = createProjectAction(
      { kind: 'docs', label: 'Open docs', url: 'https://example.com/docs' },
      'action-1',
    )!;

    expect(saveProjectAction([project], project.id, action)).toEqual([{ ...project, actions: [action] }]);
    const withAction = saveProjectAction([project], project.id, action)!;
    expect(updateProject(withAction, project.id, { name: 'Renamed site' })).toMatchObject([{ name: 'Renamed site' }]);
    ...
    expect(createProjectAction({ kind: 'custom', label: 'Unsafe', url: 'data:text/html,nope' }, 'unsafe')).toBeNull();
  });
  ...
  it('builds a deliberate workspace launch plan from valid items', () => {
    ...
    expect(updateWorkspace(complete, workspace.id, { name: 'Evening' })).toMatchObject([{ name: 'Evening' }]);
