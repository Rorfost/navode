...
describe('productivity utilities', () => {
  ...
  it('calculates an active timer after refresh and supports pause/resume', () => {
    ...
    expect(getFocusTimerSnapshot(started, new Date('2026-01-01T10:05:30.000Z'))).toEqual({
    ...
    expect(resumeFocusTimer(paused, new Date('2026-01-01T11:00:00.000Z'))?.endsAt).toBe(
      '2026-01-01T11:19:30.000Z',
    );
  });

  it('limits today to three items and preserves completion state', () => {
    let items: TodayItem[] = [];
    for (let index = 0; index < MAX_TODAY_ITEMS; index += 1)
      items = addTodayItem(items, `Priority ${index}`, `today-${index}`)!;
    expect(addTodayItem(items, 'Too many', 'today-4')).toBeNull();
    expect(toggleTodayItem(items, 'today-1')[1]).toMatchObject({
      completed: true,
      title: 'Priority 1',
    });
  });
