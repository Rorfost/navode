...
V2.2 activates the GitHub provider. V2.3 activates Google Calendar in the extension through the official `chrome.identity` OAuth flow. The host requests `identity` and the `www.googleapis.com` optional origin only after an explicit connect action, and asks solely for `calendar.events.readonly`. Chrome retains the access token in its memory-backed identity cache; Navode does not persist it. The web companion remains local-only for Calendar because it has no OAuth client configuration. The remaining providers stay planned. The core package must not depend directly on React or Chrome APIs.

## Data and deployment

The extension owns its settings in `chrome.storage.local` through `apps/extension/src/storage.ts`; web storage is browser-local. Theme, onboarding completion, reduced-motion and home preferences, default provider, aliases, bounded non-sensitive action history, links, projects, workspaces, productivity data, integration connection metadata, and provider caches use a shared versioned settings model in `packages/core`, with host-specific persistence adapters. Schema v1 through v5 migrate deterministically to the current v6 before use. V2.2 adds an optional validated `owner/repository` reference to projects and GitHub status cache data. V2.3 adds a five-minute, bounded local cache of today's Calendar event titles, all-day state, times, links, and time zone. Integration credentials are not part of this settings record. The hosts execute only resolver-produced, validated actions; workspace launches are user initiated and confirmed without extra Chrome permissions. Export/import is the portability and recovery path. The web app and API target Cloudflare later, but no production deployment is configured.
