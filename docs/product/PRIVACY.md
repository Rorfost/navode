# Navode V1 privacy policy

This is the source policy for the deployed `/privacy` page. Last updated September 19, 2026.

Navode V1 stores the configuration and content a person creates: theme and motion preferences, search provider, aliases, quick links, projects, workspaces, scratchpad text, snippets, focus timer state, Today priorities, and a bounded recent-action list. Recent actions contain labels and timestamps, not search queries or snippet contents.

The extension uses Chrome local storage; the optional web companion uses browser local storage. V1 has no account system, cloud sync, Navode-hosted database, analytics, telemetry, or advertising identifiers. Navode does not send configuration, notes, snippets, or activity to Navode servers.
