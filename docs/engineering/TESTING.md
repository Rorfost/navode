...
Use Vitest for domain and API unit tests, React Testing Library for component tests, and Playwright for important web flows. The web foundation includes a component test and a new-tab shell browser test. Test core command parsing, alias resolution, result ranking, URL validation, fallback search, and bounded history independently from React and Chrome APIs. Extension checks include storage-adapter tests, manifest validation, a production build, unpacked loading, new-tab override, and permission review.
